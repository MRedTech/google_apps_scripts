...
function setupSearchRecordSheet() {
  ...
  sh.getRange('F11')
    ...
    .setBorder(true, false, true, false, false, false, '#c8e6c9', SpreadsheetApp.BorderStyle.SOLID);
...
function setupSearchRecordActionCheckboxes_(sheet) {
  // Same checkbox concept as the Dashboard refresh checkbox:
  // normal Google Sheets checkbox, FALSE by default, no custom TRUE/FALSE validation.
...
function handleSearchRecordCheckboxEdit_(e) {
  ...
  // Same stable concept as the dashboard refresh checkbox:
  // action runs only when the checkbox becomes TRUE.
