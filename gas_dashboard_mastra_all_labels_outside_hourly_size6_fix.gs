const DASHBOARD_CONFIG = {
  sourceSheetName: 'M ASTRA',
  dashboardSheetName: 'DASHBOARD',
  timezone: 'Asia/Kuala_Lumpur',
  headerTitle: 'SECURE ENTRY DASHBOARD | M ASTRA',
...
function buildDashboard() {
  ...
  if (!sourceSheet) throw new Error("Source sheet 'M ASTRA' was not found.");
...
function applyMAstraProtection_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(DASHBOARD_CONFIG.sourceSheetName);
  if (!sheet) throw new Error("M ASTRA sheet was not found.");
  ...
  const protection = sheet.protect().setDescription('Lock M ASTRA sheet');
