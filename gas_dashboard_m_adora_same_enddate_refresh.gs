...
function onSelectionChange(e) {
  try {
    ...
    const dashboardName = DASHBOARD_CONFIG.dashboardSheetName;
    ...
    if (sheet.getName() !== dashboardName) {
    ...
    // If user leaves End Date cell after interacting with it,
    // refresh even when the date value stays the same.
    if (lastSelection === endCell && props.getProperty('SE_ENDDATE_ARMED') === '1') {
      props.deleteProperty('SE_ENDDATE_ARMED');
      props.setProperty('SE_LAST_SELECTION_A1', currentA1);

      // Skip duplicate refresh when onEdit has already refreshed D3.
      if (endEdited) {
        props.deleteProperty('SE_ENDDATE_EDITED');
        return;
      }
...
function enforceOpenSecurity_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const dashboardSheet = ss.getSheetByName(DASHBOARD_CONFIG.dashboardSheetName);
  const mAdoraSheet = ss.getSheetByName(DASHBOARD_CONFIG.sourceSheetName);

  applyDashboardProtection_();
  applyMAdoraProtection_();

  if (mAdoraSheet && !mAdoraSheet.isSheetHidden()) {
    mAdoraSheet.hideSheet();
  }
