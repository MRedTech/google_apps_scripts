...
function setupSearchRecordActionCheckboxes_(sheet) {
  ...
  // Use normal Google Sheets checkbox values (TRUE/FALSE), same as dashboard.
...
function resetSearchRecordActionCheckboxes_(sheet) {
  // Fully restore the checkbox controls after each action.
  // This prevents any typed FALSE text from remaining in F4/F5 on tablet/keyboard use.
  restoreSearchRecordActionCheckboxes_(sheet);
}
...
function handleSearchRecordCheckboxEdit_(e) {
  ...
  // Same stable concept as dashboard: action only runs when checkbox becomes TRUE.
  // Mouse/touch click and keyboard Enter/Space on the checkbox normally toggle it to TRUE.
