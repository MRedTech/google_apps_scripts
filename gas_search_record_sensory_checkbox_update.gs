...
function setupSearchRecordSheet() {
  ...
  sh.getRange(
    ...
    1,
    SEARCH_RECORD_CONFIG.historyColumnCount
  ).setValues([[
    'TIMESTAMP',
    'NAME',
    'MYKAD / PASSPORT',
...
function setSearchRecordActionBusy_(sheet, action) {
  ...
  if (action === 'SEARCH') {
    ...
    clearBox.clearDataValidations().clearContent().setBackground('#E0E0E0');
  } else {
    clearBox.setValue(true).setBackground('#D9EAD3');
    searchBox.clearDataValidations().clearContent().setBackground('#E0E0E0');
  }
...
function renderSearchRecord_(sheet, result) {
  ...
  if (values.length) {
    ...
    sheet.getRange(startRow, startCol, values.length, SEARCH_RECORD_CONFIG.historyColumnCount).setValues(values);
    ...
    sheet.getRange(startRow, startCol + 5, values.length, 1)
      .setNumberFormat('@')
      .setHorizontalAlignment('left');
...
function applyHistoryRowColors_(sheet, matches) {
  ...
  const colors = matches.map(function (item) {
    ...
    return [bg, bg, bg, bg, bg, bg, bg, bg, bg, bg];
  });
...
If this Apps Script project already has an onEdit(e) function, add this line at the top of that existing onEdit(e):

  if (handleSearchRecordCheckboxEdit_(e)) return;

Do not create a second onEdit(e) function in the same project.
*/
