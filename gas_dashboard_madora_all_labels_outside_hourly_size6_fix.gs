const DASHBOARD_CONFIG = {
  sourceSheetName: 'M ADORA',
  dashboardSheetName: 'DASHBOARD',
  timezone: 'Asia/Kuala_Lumpur',
  headerTitle: 'SECURE ENTRY DASHBOARD | M ADORA',
...
function buildDashboard() {
  ...
  if (!sourceSheet) throw new Error("Source sheet 'M ADORA' was not found.");
...
function applyMAdoraProtection_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(DASHBOARD_CONFIG.sourceSheetName);
  if (!sheet) throw new Error("M ADORA sheet was not found.");
  ...
  const protection = sheet.protect().setDescription('Lock M ADORA sheet');
