...
const SYNC_TOKEN = "vqjscBw7IFxaIREWT07pKdHY6zVsCQpm2G6zjxQOmaApTeLUlm_0-gpLMWtsOyvl";
...
function fetchAndStoreImage_(imageViewUrl, data) {
  ...
  const safeName =
    normalizeText_(data.namePassport) ||
    normalizeText_(data.regnum) ||
    "PHOTO";

  const filename =
    safeName.replace(/[\\/:*?"<>|]/g, " ").replace(/\s+/g, " ").trim() +
    "_" +
    Date.now() +
    ".jpg";
...
function cleanupByAge_(sheet, retentionDays) {
  ...
  // Worker also sends DELETE_DRIVE; repeating setTrashed(true) is harmless.
  const photoValues = sheet
    .getRange(2, COL.PHOTO, rowsToDelete, 1)
    .getValues();

  const nameFormulas = sheet
    .getRange(2, COL.NAME, rowsToDelete, 1)
    .getFormulas();

  for (let i = 0; i < rowsToDelete; i++) {
    let fileId = extractDriveFileId_(photoValues[i][0]);

    if (!fileId) {
      const formula = toText_(nameFormulas[i][0]);
      const match = formula.match(/HYPERLINK\(\s*"([^"]+)"/i);
      if (match && match[1]) {
        fileId = extractDriveFileId_(match[1]);
      }
    }
...
function doPost(e) {
  ...
  try {
    ...
    if (action === "DELETE_DRIVE") {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        try {
          ...
          // Ignore individual missing/already-trashed files.
        }
      }
    ...
    // ==================================================
    // Fetch NEW R2 image and store it in Drive BEFORE Sheet lock.
    // If no imageViewUrl, this is a reused-photo registration;
    // PHOTO LINK remains blank for this row.
    // ==================================================
    ...
    // ==================================================
    // Critical section: append Sheet row + cleanup old rows.
    // ==================================================
    lock = LockService.getScriptLock();
    ...
    if (newDriveFileId && !rowCommitted) {
      try {
        DriveApp.getFileById(newDriveFileId).setTrashed(true);
      } catch (cleanupErr) {}
    }
