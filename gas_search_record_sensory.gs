...
function getSearchInputDisplayRange_(sheet) {
  // SEARCH INPUT is designed as a merged C4:E4 field.
  // Formatting the full display range prevents copied cell styles from damaging the input line.
  return sheet.getRange('C4:E4');
}

function restoreSearchInputStyle_(sheet, isPlaceholder) {
  const displayRange = getSearchInputDisplayRange_(sheet);

  // Repair the merged input field and its border after normal paste / formatted paste.
  displayRange
    .breakApart()
    .merge()
    .setNumberFormat('@')
...
function applySearchInputPlaceholder_(sheet) {
  ...
  restoreSearchInputStyle_(sheet, true);
  range.setNumberFormat('@');

  if (!current) {
    range.setValue(SEARCH_INPUT_PLACEHOLDER);
  }
}
...
function clearSearchInputPlaceholder_(sheet) {
  ...
  restoreSearchInputStyle_(sheet, false);
  range.setNumberFormat('@');
