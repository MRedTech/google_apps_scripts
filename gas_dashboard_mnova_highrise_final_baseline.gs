const DASHBOARD_CONFIG = {
  sourceSheetName: 'M NOVA',
  dashboardSheetName: 'DASHBOARD',
  timezone: 'Asia/Kuala_Lumpur',
  headerTitle: 'SECURE ENTRY DASHBOARD | M NOVA',
...
function buildDashboard() {
  ...
  if (!sourceSheet) throw new Error("Source sheet 'M NOVA' was not found.");
...
function buildCharts_(sheet, stats) {
  ...
  const towerMax = roundUpToStep_(addChartHeadroom_(maxTower, 0.2), towerStep);
...
function applyMNovaProtection_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(DASHBOARD_CONFIG.sourceSheetName);
  if (!sheet) throw new Error("M NOVA sheet was not found.");
  ...
  const protection = sheet.protect().setDescription('Lock M NOVA sheet');
