const DASHBOARD_CONFIG = {
  sourceSheetName: 'M ARISA',
  dashboardSheetName: 'DASHBOARD',
  timezone: 'Asia/Kuala_Lumpur',
  headerTitle: 'SECURE ENTRY DASHBOARD | M ARISA',
...
function buildDashboard() {
  ...
  if (!sourceSheet) throw new Error("Source sheet 'M ARISA' was not found.");
...
function applyMArisaProtection_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(DASHBOARD_CONFIG.sourceSheetName);
  if (!sheet) throw new Error("M ARISA sheet was not found.");
  ...
  const protection = sheet.protect().setDescription('Lock M ARISA sheet');
