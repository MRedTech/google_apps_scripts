...
function setupSearchRecordSheet() {
  ...
  sh.setColumnWidth(6, 20); // dedicated narrow checkbox / spacer column, same concept as dashboard
...
function setupSearchRecordActionCheckboxes_(sheet) {
  ...
  // Lowercase custom values are used to avoid tablet/keyboard validation popups
  // when the cell value appears as false/true in the formula bar.
...
function handleSearchRecordCheckboxEdit_(e) {
  ...
  // Action runs when the checkbox becomes TRUE.
  // On some tablets/keyboards, pressing Enter on the checkbox can submit the visible
  // unchecked value (false). Treat that as a keyboard action for this control only.
  const checkboxValue = String(range.getValue() || '').toUpperCase();
  const eventValue = String(e.value || '').toUpperCase();
  const oldEventValue = String(e.oldValue || '').toUpperCase();
  const isChecked = checkboxValue === 'TRUE' || eventValue === 'TRUE';
  const isKeyboardEnterAction = eventValue === 'FALSE' && oldEventValue !== 'TRUE';

  if (!isChecked && !isKeyboardEnterAction) {
    return true;
  }
  ...
  try {
    ...
    // Keep only one checkbox active at a time.
    if (isSearchAction) {
      sheet.getRange(SEARCH_RECORD_CONFIG.actionClearCell).setValue('false');
      SpreadsheetApp.flush();
      runSearchRecord();
    ...
    try {
      ...
      Logger.log('Search Record checkbox reset error: ' + resetErr);
    }
    throw err;
