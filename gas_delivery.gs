...
function ensureSyncMetaSheet_(mainSheet) {
  ...
  const expectedHeaders = [["SYNC ID", "DRIVE FILE ID", "DRIVE URL", "TIMESTAMP"]];
  const currentHeaders = metaSheet.getRange(1, 1, 1, 4).getDisplayValues()[0].map(v => toText_(v).toUpperCase());
  const expectedUpper = expectedHeaders[0].map(v => v.toUpperCase());
  ...
  if (emptyHeader) {
    ...
    for (let i = 0; i < 4; i++) {
      if (currentHeaders[i] !== expectedUpper[i]) {
        throw new Error("SYNC META header mismatch at column " + (i + 1));
      }
    }
  }
  ...
  if (mainSheet.getMaxColumns() >= 11) {
    ...
    if (jHeader === "SYNC ID" && kHeader === "DRIVE FILE ID") {
      ...
      if (lastRow > 1) {
        ...
        if (metaLastRow > 1) {
          metaSheet.getRange(2, 1, metaLastRow - 1, 1).getDisplayValues().forEach(r => {
            const v = toText_(r[0]);
        ...
        legacy.forEach(r => {
          ...
          rowsToAdd.push([syncId, driveFileId, driveUrl, ts]);
...
function findSyncRecordById_(metaSheet, reqId) {
  ...
  const driveFileId = toText_(vals[1]);
  let driveUrl = toText_(vals[2]);
...
function cleanupSyncMetaByAge_(metaSheet, retentionDays) {
  ...
  const tsVals = metaSheet.getRange(2, 4, lastRow - 1, 1).getDisplayValues();
  const rowsToDelete = [];

  for (let i = 0; i < tsVals.length; i++) {
    const dt = parseTimestampDMY_(tsVals[i][0]);
    if (dt && dt < cutoff) rowsToDelete.push(i + 2);
  }
...
function doPost(e) {
  ...
  try {
    ...
    if (action === "CLEANUP_AGE") {
      ...
      return output_({ success: true, action: "CLEANUP_AGE", cleaned: !!did, syncMetaCleaned: !!didMeta });
    }
    ...
    const timestampText = formatDateTimeDMY(now);
    const rowValues = [[
