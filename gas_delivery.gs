...
const SYNC_ID_COL = 10;       // J - durable Worker record id for idempotency
const DRIVE_FILE_ID_COL = 11; // K - Drive file id created for this sync (blank for RECORD FOUND reuse)
...
function ensureIdempotencyColumns_(sheet) {
  if (sheet.getMaxColumns() < DRIVE_FILE_ID_COL) {
    sheet.insertColumnsAfter(sheet.getMaxColumns(), DRIVE_FILE_ID_COL - sheet.getMaxColumns());
  }

  const syncHeaderCell = sheet.getRange(1, SYNC_ID_COL);
  const driveHeaderCell = sheet.getRange(1, DRIVE_FILE_ID_COL);
  const syncHeader = toText_(syncHeaderCell.getDisplayValue()).toUpperCase();
  const driveHeader = toText_(driveHeaderCell.getDisplayValue()).toUpperCase();

  if (!syncHeader || /^COLUMN\s*10$/i.test(syncHeader)) {
    syncHeaderCell.setValue("SYNC ID");
  } else if (syncHeader !== "SYNC ID") {
    throw new Error("Column J is already in use. Expected header: SYNC ID");
  }

  if (!driveHeader || /^COLUMN\s*11$/i.test(driveHeader)) {
    driveHeaderCell.setValue("DRIVE FILE ID");
  } else if (driveHeader !== "DRIVE FILE ID") {
    throw new Error("Column K is already in use. Expected header: DRIVE FILE ID");
  }

  try { sheet.hideColumns(SYNC_ID_COL, 2); } catch (hideErr) {}
}

function findSyncRecordById_(sheet, reqId) {
  const id = toText_(reqId);
  const lastRow = sheet.getLastRow();
  if (!id || lastRow <= 1) return null;

  const match = sheet
    .getRange(2, SYNC_ID_COL, lastRow - 1, 1)
    .createTextFinder(id)
  ...
  const driveFileId = toText_(sheet.getRange(row, DRIVE_FILE_ID_COL).getDisplayValue());
  let driveUrl = toText_(sheet.getRange(row, 9).getDisplayValue());
...
function doPost(e) {
  ...
  try {
    ...
    // - Column J is the durable source of truth.
    // =========================
    ...
    const existing = findSyncRecordById_(sheet, reqId);
    if (existing) {
      try {
    ...
    // A-K are committed in one Sheet write so the durable SYNC ID
    // always exists at the same time as the registration row.
    const rowValues = [[
      ...
      reqId,                             // J SYNC ID (durable idempotency key)
      driveFileId                        // K DRIVE FILE ID (blank when no new photo)
    ]];
    ...
    // Fast helper only; durable authority is column J.
    try {
      cache.put(doneKey, JSON.stringify({
        driveFileId: driveFileId,
        driveUrl: photoUrl
