const DASHBOARD_CONFIG = {
  ...
  filterStartCell: 'B3',
  filterEndCell: 'D3',
  refreshLabelRange: 'F3:G3',
  refreshCheckboxCell: 'E3',
  helperStartColumn: 14, // N
  helperStartRow: 75,
  d1DashboardApiUrl: 'https://mredtech01.edreborn86.workers.dev/dashboard/delivery'
};
...
const GROUP_CATEGORY_CHART_ORDER = [
  'FOOD DELIVERY',
  'E-HAILING / DROP OFF / PICK UP',
  'PARCEL / COURIER'
];
...
function refreshDashboard() {
  ...
  if (!dashboardSheet) {
    throw new Error("Dashboard sheet 'DASHBOARD' was not found. Please run buildDashboard() first.");
  }

  const dateFilter = getDashboardDateFilterFromCells_(dashboardSheet);
  const d1Data = fetchDashboardDataFromD1_(dateFilter.startYmd, dateFilter.endYmd);
  const stats = transformD1DashboardResponse_(d1Data);
...
function setupRefreshCheckbox_(sheet) {
  // Clear the current and previous refresh control area before placing the checkbox and label.
  const refreshArea = sheet.getRange('E3:H3');
  refreshArea
    .breakApart()
    .clearContent()
  ...
  const checkboxCell = sheet.getRange(DASHBOARD_CONFIG.refreshCheckboxCell);
  ...
  sheet.getRange(DASHBOARD_CONFIG.refreshLabelRange)
    .breakApart()
    .merge()
...
function getDashboardDateFilterFromCells_(sheet) {
  const today = toDateOnly_(new Date());

  let start = parseDashboardFilterDate_(sheet, DASHBOARD_CONFIG.filterStartCell) || today;
  let end = parseDashboardFilterDate_(sheet, DASHBOARD_CONFIG.filterEndCell) || today;
...
function fetchDashboardDataFromD1_(startYmd, endYmd) {
  const baseUrl = safeString_(DASHBOARD_CONFIG.d1DashboardApiUrl).replace(/\/+$/, '');
  if (!baseUrl) {
    throw new Error('D1 Dashboard API URL is not configured.');
  }
  ...
  try {
    ...
    throw new Error('D1 Dashboard API returned an invalid response.');
  }

  if (statusCode < 200 || statusCode >= 300 || !payload || payload.success !== true) {
    const message = payload && payload.message
      ? payload.message
      : 'D1 Dashboard request failed (' + statusCode + ').';
...
function transformD1DashboardResponse_(payload) {
  const summary = payload.summary || {};
  const hourlyRows = Array.isArray(payload.hourlyTrend) ? payload.hourlyTrend : [];
  const monthlyRows = Array.isArray(payload.monthlyTrend) ? payload.monthlyTrend : [];
  const categoryRows = Array.isArray(payload.categoryBreakdown) ? payload.categoryBreakdown : [];
  const groupRows = Array.isArray(payload.groupCategoryBreakdown) ? payload.groupCategoryBreakdown : [];
  const towerData = payload.towerDistribution || {};
  const towerRows = Array.isArray(towerData.rows) ? towerData.rows : [];
  const statusRows = Array.isArray(payload.recordStatus) ? payload.recordStatus : [];

  const hourlyMap = new Map();
  hourlyRows.forEach(item => {
    const hour = normalizeDashboardHour_(item && item.hour);
    if (hour) {
      hourlyMap.set(hour, Number(item.total || 0));
    }
  });
  ...
  for (let hour = 0; hour < 24; hour++) {
    ...
    hourlyTrend.push([label, hourlyMap.get(label) || 0]);
  }

  const monthly = monthlyRows.map(item => [
    safeUpper_(item && item.month),
    Number(item && item.foodDelivery || 0),
    Number(item && item.eHailing || 0),
    Number(item && item.parcelCourier || 0)
  ]);

  const category = categoryRows.map(item => [
    normalizeCategory_(item && item.category),
    Number(item && item.total || 0)
  ]);

  const groupMap = new Map();
  groupRows.forEach(item => {
    const group = safeUpper_(item && item.group);
    if (group) {
      groupMap.set(group, Number(item.total || 0));
    }
  });

  const groupCategory = GROUP_CATEGORY_CHART_ORDER.map(group => [
    group,
    groupMap.get(group) || 0
  ]);

  const towerMap = new Map();
  towerRows.forEach(item => {
    const group = safeUpper_(item && item.group);
    if (group) {
      towerMap.set(group, {
        towerA: Number(item.towerA || 0),
        towerB: Number(item.towerB || 0)
      });
    }
  });
  ...
  const towerDistribution = [
    ...
    ...TOWER_GROUP_ORDER.map(group => {
      const values = towerMap.get(group) || { towerA: 0, towerB: 0 };
      return [group, values.towerA, values.towerB];
    })
  ];

  const statusMap = new Map();
  statusRows.forEach(item => {
    const status = safeUpper_(item && item.status);
    if (status) {
      statusMap.set(status, Number(item.total || 0));
    }
  });

  const recordStatus = [
    ['NEW RECORD', statusMap.get('NEW RECORD') || 0],
    ['EXISTING RECORD', statusMap.get('EXISTING RECORD') || 0]
  ];

  const totalFiltered = Number(summary.totalRegistration || 0);
  const foundRateValue = normalizeRateNumber_(summary.recordFoundRate);
  const newRateValue = normalizeRateNumber_(summary.newRecordRate);

  return {
    totalFiltered,
    peakHour: normalizeDashboardHour_(summary.peakHour) || '-',
    foundRate: foundRateValue + '%',
    ...
    towerDistribution,
    recordStatus,
    lastUpdated: safeString_(payload.lastUpdated)
  };
}

function normalizeDashboardHour_(value) {
...
function normalizeRateNumber_(value) {
  const numberValue = Number(String(value == null ? 0 : value).replace('%', ''));
...
function buildStatistics_(filteredRows, allRows) {
  ...
  const groupCategory = aggregateGroupCategoryFixedOrder_(statsRows);
...
function aggregateGroupCategoryFixedOrder_(rows) {
  const result = Object.fromEntries(
    GROUP_CATEGORY_CHART_ORDER.map(group => [group, 0])
  );
  ...
  return GROUP_CATEGORY_CHART_ORDER.map(group => [
    group,
    result[group] || 0
  ]);
}
...
function addTowerAnnotationRows_(rows) {
  return rows.map(row => [
    row[0],
    row[1] || 0,
    String(row[1] || 0),
...
function addChartHeadroom_(value, percent) {
  if (!value || value < 0) return 0;
  return Math.ceil(value * (1 + percent));
}

function writeDashboardValues_(sheet, stats, dateFilter) {
  let lastUpdatedDate = stats.lastUpdated ? new Date(stats.lastUpdated) : new Date();
  if (isNaN(lastUpdatedDate.getTime())) {
    lastUpdatedDate = new Date();
  }

  sheet.getRange('K2').setValue(
    Utilities.formatDate(lastUpdatedDate, DASHBOARD_CONFIG.timezone, 'dd/MM/yyyy HH:mm:ss')
  );
...
function writeHelperTables_(sheet, stats) {
  ...
  const tables = [
    ...
    { title: 'CATEGORY', headers: ['CATEGORY', 'TOTAL'], rows: stats.category, colOffset: 7 },
...
function buildCharts_(sheet, stats) {
  ...
  const categoryRange = sheet.getRange(startRow + 1, startCol + 7, Math.max(stats.category.length + 1, 2), 2);
  ...
  const monthlyStep = getBalancedAutoStep_(maxMonthly);
  ...
  const hourlyLabelFontSize = maxHourly >= 1000 ? 6 : 10;
  ...
  const charts = [
    sheet.newChart()
      ...
      .setOption('dataLabelPlacement', 'outsideEnd')
      .setOption('dataLabelFontSize', hourlyLabelFontSize)
      .setOption('annotations', {
        alwaysOutside: true,
        textStyle: {
          fontSize: hourlyLabelFontSize,
          color: '#4285F4',
          auraColor: 'none'
        }
      })
      .setOption('series', {
        0: {
          ...
          dataLabelFontSize: hourlyLabelFontSize,
          textStyle: { fontSize: hourlyLabelFontSize }
        }
      })
      .setOption('series.0.dataLabelPlacement', 'outsideEnd')
      .setOption('series.0.textStyle.fontSize', hourlyLabelFontSize)
    ...
    sheet.newChart()
      ...
      .setOption('dataLabelPlacement', 'outsideEnd')
      .setOption('series', {
        0: { color: '#DB4437', dataLabel: 'value', dataLabelPlacement: 'outsideEnd' },
        1: { color: '#4285F4', dataLabel: 'value', dataLabelPlacement: 'outsideEnd' },
        2: { color: '#F4B400', dataLabel: 'value', dataLabelPlacement: 'outsideEnd' }
      })
      .setOption('series.0.dataLabelPlacement', 'outsideEnd')
      .setOption('series.1.dataLabelPlacement', 'outsideEnd')
      .setOption('series.2.dataLabelPlacement', 'outsideEnd')
      .setOption('vAxis', {
        textStyle: { fontSize: 10 },
    ...
    sheet.newChart()
      ...
      .setOption('dataLabelPlacement', 'outsideEnd')
      .setOption('series', {
        0: { color: '#4285F4', dataLabel: 'value', dataLabelPlacement: 'outsideEnd' }
      })
      .setOption('series.0.dataLabelPlacement', 'outsideEnd')
      .setOption('hAxis', {
    ...
    sheet.newChart()
      ...
      .setOption('colors', ['#DB4437', '#4285F4', '#F4B400'])
    ...
    sheet.newChart()
      ...
      .setOption('series', {
        0: { color: '#4285F4', dataLabel: 'value', dataLabelPlacement: 'outsideEnd' },
        1: { color: '#0F9D58', dataLabel: 'value', dataLabelPlacement: 'outsideEnd' }
      })
      .setOption('series.0.dataLabelPlacement', 'outsideEnd')
      .setOption('series.1.dataLabelPlacement', 'outsideEnd')
      .setOption('hAxis', {
...
function getBalancedAutoStep_(maxValue) {
  if (!maxValue || maxValue <= 0) return 1;

  const targetGridlines = 7;
  const rawStep = maxValue / targetGridlines;
  const power = Math.pow(10, Math.floor(Math.log(rawStep) / Math.LN10));
  const scaledStep = rawStep / power;

  if (scaledStep <= 1) return power;
  if (scaledStep <= 2) return 2 * power;
  if (scaledStep <= 5) return 5 * power;
  return 10 * power;
}
...
function testDashboardD1Connection() {
  const today = ymdDate_(new Date());
  const payload = fetchDashboardDataFromD1_(today, today);

  SpreadsheetApp.getActiveSpreadsheet().toast(
    'D1 connection successful. Total registration: '
      + Number(payload.summary && payload.summary.totalRegistration || 0),
    'Secure Entry Dashboard',
    8
  );
}
...
function installDashboardEditTrigger() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  removeDashboardEditTriggers_();

  ScriptApp.newTrigger('handleDashboardEdit')
    .forSpreadsheet(ss)
    .onEdit()
    .create();

  SpreadsheetApp.getUi().alert('Dashboard edit trigger installed successfully.');
}

function removeDashboardEditTriggers_() {
  const triggers = ScriptApp.getProjectTriggers();

  triggers.forEach(trigger => {
    const handler = trigger.getHandlerFunction();
    if (handler === 'handleDashboardEdit') {
...
function handleDashboardEdit(e) {
  try {
    ...
    // =========================
    // DASHBOARD REFRESH CHECKBOX LOGIC
    // Install this handler using installDashboardEditTrigger().
    // Do not rename this function to onEdit(), because a simple onEdit trigger
    // runs under the editing user's permission and may fail on protected cells.
    // =========================
    if (sheetName === DASHBOARD_CONFIG.dashboardSheetName) {
      if (a1 !== DASHBOARD_CONFIG.refreshCheckboxCell) return;

      const checkboxValue = sheet.getRange(DASHBOARD_CONFIG.refreshCheckboxCell).getValue();
      ...
      try {
        ...
        // Move the active selection away from the refresh checkbox before the dashboard refresh starts.
        sheet.getRange('A1').activate();
        SpreadsheetApp.flush();

        // Refresh first, then reset the checkbox after the dashboard update is completed.
        refreshDashboard();
        SpreadsheetApp.flush();

        sheet.getRange(DASHBOARD_CONFIG.refreshCheckboxCell).setValue(false);
        SpreadsheetApp.flush();
        ...
        try {
          ...
          Logger.log('Refresh checkbox reset/selection error: ' + resetError);
        }
    ...
    Logger.log('handleDashboardEdit error: ' + err);
  }
}
...
function onSelectionChange(e) {
  try {
    ...
    // Date cells no longer trigger dashboard refresh.
    // Dashboard refresh now runs only when the refresh checkbox is ticked.
    // =========================
    if (sheetName === DASHBOARD_CONFIG.dashboardSheetName) {
...
function enforceOpenSecurity_() {
  ...
  if (sourceSheet && !sourceSheet.isSheetHidden()) {
  ...
  if (dashboardSheet) {
    ...
    setupRefreshCheckbox_(dashboardSheet);
    styleFilterCell_(dashboardSheet.getRange(DASHBOARD_CONFIG.filterEndCell));
...
function applyDashboardProtection_() {
  ...
  const protection = sheet.protect().setDescription('Lock dashboard except B3, D3 and E3');

  protection.setUnprotectedRanges([
    sheet.getRange(DASHBOARD_CONFIG.filterStartCell),
    sheet.getRange(DASHBOARD_CONFIG.filterEndCell),
    sheet.getRange(DASHBOARD_CONFIG.refreshCheckboxCell)
  ]);
