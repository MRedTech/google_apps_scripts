...
function appendNoticeInfoTable_(body, rows) {
  ...
  for (let r = 0; r < table.getNumRows(); r++) {
    ...
    for (let c = 0; c < row.getNumCells(); c++) {
      ...
      paragraph.setSpacingBefore(2).setSpacingAfter(2);
      const text = paragraph.editAsText().setFontSize(9);
      if (c === 0) {
        text.setBold(true).setForegroundColor("#5E7465");
        cell.setBackgroundColor("#EEF7F0");
      } else {
        text.setBold(true).setForegroundColor("#173C28");
      }
    }
  }
  return table;
...
function createParkingNoticePdf_(data) {
  ...
  styleParagraph_(
    body.appendParagraph("OVERNIGHT PARKING CHARGE NOTICE"),
    {
      align: DocumentApp.HorizontalAlignment.CENTER,
      bold: true,
      size: 14,
  ...
  styleParagraph_(
    body.appendParagraph("NOTICE NO.: " + noticeNumber),
    {
      align: DocumentApp.HorizontalAlignment.CENTER,
      bold: true,
      size: 10,
  ...
  appendNoticeInfoTable_(body, [
    ...
    ["FIRST 24 HOURS", "FREE"],
    ["CHARGEABLE BLOCKS", String(Math.max(0, Number(data.chargeBlocks) || 0))],
    ["RATE", moneyFromCents_(data.rateCents) + " PER STARTED 24-HOUR BLOCK"],
    ["TOTAL AMOUNT DUE", moneyFromCents_(data.totalChargeCents)]
  ]);

  body.appendParagraph("");
  styleParagraph_(body.appendParagraph("PARKING ACTIVITY LOG - LATEST ACTIVITY FIRST"), {
    bold: true,
    size: 11,
    color: "#176B39"
  });
  ...
  const activityTable = body.appendTable(activityRows);
  for (let r = 0; r < activityTable.getNumRows(); r++) {
    const row = activityTable.getRow(r);
    for (let c = 0; c < row.getNumCells(); c++) {
      const cell = row.getCell(c);
      const paragraph = cell.getChild(0).asParagraph();
      paragraph.setSpacingBefore(1).setSpacingAfter(1);
      const text = paragraph.editAsText().setFontSize(7);
      if (r === 0) {
        text.setBold(true).setForegroundColor("#FFFFFF");
        cell.setBackgroundColor("#176B39");
      } else {
        text.setForegroundColor("#243F2E");
      }
    }
  }
  ...
  styleParagraph_(
    body.appendParagraph(
      "PAYMENT AND FURTHER ARRANGEMENTS WILL BE MANAGED BY THE MANAGEMENT OFFICE WITH THE UNIT OWNER."
    ),
    { bold: true, size: 9, color: "#176B39" }
  );
  ...
  styleParagraph_(
    ...
    { bold: true, size: 10, color: "#8B2E1B" }
  );
  ...
  body.appendParagraph("");
  styleParagraph_(
    body.appendParagraph("POWERED BY MRED TECH"),
    {
      align: DocumentApp.HorizontalAlignment.CENTER,
      bold: true,
      size: 8,
...
function parkingNoticeEmailBody_(data) {
  return [
    ...
    "Payment and further arrangements are to be managed by PMO with the unit owner.",
    "Payment is not collected at the guard house.",
    "",
