...
function doPost(e) {
  ...
  try {
    ...
    if (isDefaulterAudit) {
      // Dedicated DEFAULTER audit mode must remain identity/photo free and the
      // reason is owned by the mode itself.
      if (namePassport || mykadPassport) {
        return output_({
          success: false,
          error: true,
          retryable: false,
          message: "DEFAULTER mode must not include identity fields."
        });
      }
      ...
      if (toText_(data.imageViewUrl)) {
        return output_({
          ...
          error: true,
          retryable: false,
          message: "DEFAULTER mode must not include an ID photo."
        });
      }
    } else {
