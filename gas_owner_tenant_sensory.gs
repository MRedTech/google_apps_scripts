...
function doPost(e) {
  ...
  try {
    ...
    // Finding #6: mode is now explicit in the Worker -> GAS contract.
    // Temporary blank-mode fallback keeps rolling deployment backward compatible
    // with the previous Worker until the new Worker is deployed.
    const modeRaw = normalizeText_(data.mode);
    let mode = modeRaw;

    if (!mode) {
      // Backward-compatible fallback: DEFAULTER TENANT may now legitimately
      // include identity fields, so reason is the reliable legacy signal.
      mode = reason === "DEFAULTER" ? "DEFAULTER" : "NORMAL";
    }

    if (mode !== "NORMAL" && mode !== "DEFAULTER") {
      return output_({
        success: false,
        error: true,
        retryable: false,
    ...
    const isDefaulterAudit = mode === "DEFAULTER";
    const isDefaulterTenant = isDefaulterAudit && category === "TENANT";
    ...
    if (isDefaulterAudit) {
      // The mode owns the reason for every DEFAULTER audit entry.
      if (reason !== "DEFAULTER") {
        return output_({
          success: false,
          error: true,
          retryable: false,
          message: "DEFAULTER mode requires reason DEFAULTER."
        });
      }

      if (isDefaulterTenant) {
        // New Worker records provide BOTH identity fields. Keep both-blank
        // compatibility only so older queued DEFAULTER TENANT rows can finish
        // syncing safely during rollout; the Worker prevents new blank records.
        const hasAnyTenantIdentity = !!namePassport || !!mykadPassport;
        if (hasAnyTenantIdentity && (!namePassport || !mykadPassport)) {
          return output_({
            success: false,
            error: true,
            retryable: false,
            message: "DEFAULTER TENANT identity fields must be complete."
          });
        }
        if (!hasAnyTenantIdentity && toText_(data.imageViewUrl)) {
          return output_({
            success: false,
            error: true,
            retryable: false,
            message: "DEFAULTER TENANT photo requires identity fields."
          });
        }
      } else {
        // OWNER keeps the original Defaulter audit flow: no identity or ID photo.
        if (namePassport || mykadPassport) {
          return output_({
            success: false,
            error: true,
            retryable: false,
            message: "DEFAULTER OWNER must not include identity fields."
          });
        }

        if (toText_(data.imageViewUrl)) {
          return output_({
            success: false,
            error: true,
            retryable: false,
            message: "DEFAULTER OWNER must not include an ID photo."
          });
        }
      }
    } else {
      // NORMAL keeps the existing identity requirement unchanged and can no
      // longer use DEFAULTER as a reason; that reason belongs to the dedicated mode.
      if (!namePassport || !mykadPassport) {
        return output_({
          success: false,
          error: true,
      ...
      if (reason === "DEFAULTER") {
        return output_({
          ...
          error: true,
          retryable: false,
          message: "DEFAULTER reason requires DEFAULTER mode."
        });
      }
    }
