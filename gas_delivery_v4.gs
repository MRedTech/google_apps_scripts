...
function cleanupByAge_(sheet, retentionDays) {
  ...
  if (!firstDt || firstDt >= cutoff) return result;
  ...
  if (rowsToDelete <= 0) return result;
  ...
  // Preserve the 90-day privacy boundary even if a Drive delete is pending.
  // RET-01 keeps the corresponding SYNC META/D1 recovery reference for retry.
