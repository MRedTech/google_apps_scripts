const DASHBOARD_CONFIG = {
  ...
  filterStartCell: 'B3',
  filterEndCell: 'D3',
  refreshLabelRange: 'H3:I3',
  refreshCheckboxCell: 'G3',
...
function onEdit(e) {
  try {
    ...
    if (sheetName === DASHBOARD_CONFIG.dashboardSheetName) {
      ...
      const isRefreshCheckbox = a1 === DASHBOARD_CONFIG.refreshCheckboxCell;

      if (!isStart && !isEnd && !isRefreshCheckbox) return;

      if (isRefreshCheckbox) {
...
function applyDashboardProtection_() {
  ...
  const protection = sheet.protect().setDescription('Lock dashboard except B3, D3 and L3');
