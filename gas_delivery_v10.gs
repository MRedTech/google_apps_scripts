...
// Script Properties used as secrets: SYNC_TOKEN, DELETE_DRIVE_TOKEN, CLEANUP_AGE_TOKEN, IMAGE_VIEW_TOKEN
// Non-secret Script Property: WORKER_IMAGE_BASE_URL (exact trusted Worker HTTPS origin, no path)
...
function getDeleteDriveToken_() {
  return toText_(PropertiesService.getScriptProperties().getProperty("DELETE_DRIVE_TOKEN"));
}

function getCleanupAgeToken_() {
  return toText_(PropertiesService.getScriptProperties().getProperty("CLEANUP_AGE_TOKEN"));
}
...
function getWorkerImageBaseUrl_() {
  return toText_(PropertiesService.getScriptProperties().getProperty("WORKER_IMAGE_BASE_URL"));
}

function buildTrustedWorkerImageUrl_(syncId) {
  // NEW-SEC-05: never fetch an image URL supplied by the request payload.
  // The only allowed destination is the exact HTTPS Worker origin configured
  // by management in Script Properties. This prevents IMAGE_VIEW_TOKEN from
  // being sent to an attacker-controlled URL if SYNC_TOKEN is ever exposed.
  let base = getWorkerImageBaseUrl_();
  if (!base) throw new Error("WORKER_IMAGE_BASE_URL is not configured");

  base = base.replace(/\/+$/, "");
  if (!/^https:\/\/[A-Za-z0-9.-]+(?::\d{1,5})?$/.test(base)) {
    throw new Error("WORKER_IMAGE_BASE_URL must be an exact HTTPS origin without path/query");
  }

  const id = toText_(syncId);
  if (!id) throw new Error("Missing sync id for image fetch");

  return base + "/image?id=" + encodeURIComponent(id);
}

function authorizeActionToken_(data, action) {
  let expectedToken = "";
  let propertyName = "";

  if (action === "DELETE_DRIVE") {
    expectedToken = getDeleteDriveToken_();
    propertyName = "DELETE_DRIVE_TOKEN";
  } else if (action === "CLEANUP_AGE") {
    expectedToken = getCleanupAgeToken_();
    propertyName = "CLEANUP_AGE_TOKEN";
  } else {
    expectedToken = getSyncToken_();
    propertyName = "SYNC_TOKEN";
  }

  if (!expectedToken) {
    return { ok: false, message: propertyName + " is not configured" };
  }

  if (toText_(data && data.token) !== expectedToken) {
...
function fileBelongsToSecureEntryFolder_(file) {
  if (!file || !DRIVE_FOLDER_ID) return false;

  const parents = file.getParents();
  while (parents.hasNext()) {
    const parent = parents.next();
    if (toText_(parent.getId()) === toText_(DRIVE_FOLDER_ID)) return true;
  }
  ...
  for (let x = 0; x < expiredIndexes.length; x++) {
    ...
    if (fileId) {
      ...
      try {
        ...
        // SEC-02 residual closure:
        // cleanupByAge_ must enforce the same Drive-folder boundary as the
        // DELETE_DRIVE endpoint. Never trash an arbitrary file merely because
        // a Sheet cell contains its id.
        if (!fileBelongsToSecureEntryFolder_(file)) {
          result.driveDeleteFailed++;
          console.warn(
            "SEC-02 retention Drive cleanup rejected file outside Secure Entry folder: " +
            fileId
          );
          continue;
        }
...
function doPost(e) {
  ...
  try {
    ...
    // SEC-02 residual hardening: least-privilege action tokens.
    // SYNC_TOKEN can no longer authorize Drive deletion or manual age cleanup.
    const auth = authorizeActionToken_(data, action);
    if (!auth.ok) {
      return output_({ success: false, error: true, retryable: false, message: auth.message });
    }

    // =========================
    // Action: DELETE_DRIVE (from Worker scheduled cleanup)
    // SEC-02: requires DELETE_DRIVE_TOKEN (separate from SYNC_TOKEN).
    // RET-01 payload (preferred):
    // { token, action:"DELETE_DRIVE", items:[{syncId, driveFileId, requireDrive}] }
    // Legacy { fileIds:[...] } remains supported, but every target must still
    // be verified as a file inside this site's DRIVE_FOLDER_ID.
    // =========================
    if (action === "DELETE_DRIVE") {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (syncId) {
          ...
          const metaDriveFileId = meta ? toText_(meta.driveFileId) : "";

          // SEC-02: when durable metadata has a Drive id, it is authoritative.
          // A caller-supplied id that disagrees must never be deleted.
          if (metaDriveFileId) {
            if (driveFileId && driveFileId !== metaDriveFileId) {
              failedIds.push(responseId);
              failedDetails.push({ id: responseId, reason: "DRIVE_FILE_ID_MISMATCH" });
              continue;
            }
            driveFileId = metaDriveFileId;
          }
        }
        ...
        try {
          ...
          // SEC-02: never allow this endpoint to trash an arbitrary Drive file.
          // Current Secure Entry photos are created directly in DRIVE_FOLDER_ID.
          if (!fileBelongsToSecureEntryFolder_(file)) {
            failedIds.push(responseId);
            failedDetails.push({ id: responseId, reason: "DRIVE_FILE_OUTSIDE_SECURE_ENTRY_FOLDER" });
            continue;
          }
    ...
    // SEC-02: requires CLEANUP_AGE_TOKEN (separate from SYNC_TOKEN).
    ...
    // =========================
    const imageViewRequested = !!toText_(data.imageViewUrl);

    if (imageViewRequested) {
      // NEW-SEC-05:
      // Treat imageViewUrl from the payload only as an image-present signal.
      // Never fetch that caller-supplied URL. Construct the fetch destination
      // from the trusted Worker origin + authenticated sync id instead.
      const imageViewToken = getImageViewToken_();
      if (!imageViewToken) {
        throw new Error("IMAGE_VIEW_TOKEN is not configured");
      }

      const trustedImageViewUrl = buildTrustedWorkerImageUrl_(reqId);
      const resp = UrlFetchApp.fetch(trustedImageViewUrl, {
        method: "get",
        headers: {
...
// NEW-RET-08:
// D1 treats an identifier as NO RECORD once its latest remaining row is
// outside the retention window. Apply the same rule here before returning
// EXPIRED from the legacy GAS fallback/cache path.
const anyRow = result.anyRow;
const full = sheet.getRange(anyRow, 1, 1, 9).getValues()[0];
const anyTimestamp = toText_(full[0]);

if (isExpiredByAge_(anyTimestamp, RETENTION_DAYS)) {
  return output_({ exist: false });
}

// If the latest row is still within retention but no active proof exists,
// this is a genuine RECORD EXPIRED.
if (result.status === "EXPIRED") {
  return output_({ exist: true, hasHyperlink: false, data: {} });
}

// Latest details are already loaded above.
