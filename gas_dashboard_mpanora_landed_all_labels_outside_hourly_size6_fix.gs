const DASHBOARD_CONFIG = {
  sourceSheetName: 'M PANORA',
  dashboardSheetName: 'DASHBOARD',
  timezone: 'Asia/Kuala_Lumpur',
  headerTitle: 'SECURE ENTRY DASHBOARD | M PANORA',
...
function buildDashboard() {
  ...
  if (!sourceSheet) throw new Error("Source sheet 'M PANORA' was not found.");
...
function refreshDashboard() {
  ...
  writeTowerGroupCategoryVisible_(dashboardSheet, stats.groupCategorySummary);
...
function createSectionFrames_(sheet) {
  const sections = [
    { title: 'REGISTRATION BY GROUP CATEGORY', range: 'I5:L12', titleCell: 'I5' },
    { title: 'HOURLY REGISTRATION TREND', range: 'A14:F30', titleCell: 'A14' },
    { title: 'MONTHLY REGISTRATION TREND', range: 'G14:L30', titleCell: 'G14' },
    { title: 'CATEGORY BREAKDOWN', range: 'A32:F50', titleCell: 'A32' },
    { title: 'GROUP CATEGORY BREAKDOWN', range: 'G32:L50', titleCell: 'G32' },
    { title: 'RECORD STATUS', range: 'A52:F69', titleCell: 'A52' }
  ];
...
function buildStatistics_(filteredRows, allRows) {
  ...
  const groupCategorySummary = buildGroupCategoryTotalBreakdown_(statsRows);
...
function buildGroupCategoryTotalBreakdown_(rows) {
  const result = Object.fromEntries(ACTIVE_GROUPS.map(group => [group, 0]));
  ...
  return [
    ...
    ...ACTIVE_GROUPS.map(group => [group, result[group] || 0])
  ];
}
...
function writeTowerGroupCategoryVisible_(sheet, groupCategorySummary) {
  const startRow = 5;
  ...
  sheet.getRange(startRow, startCol, 1, totalCols)
    .merge()
    .setValue('REGISTRATION BY GROUP CATEGORY')
    .setFontWeight('bold')
    .setFontSize(11)
    .setFontColor('#1b5e20')
    .setBackground('#f1f8e9')
  ...
  sheet.getRange(startRow + 1, startCol, 1, 3)
    .merge()
  ...
  let r = startRow + 2;
  const rows = (groupCategorySummary || []).slice(1);
  ...
  const grandTotal = rows.reduce((sum, item) => sum + (item[1] || 0), 0);
...
function writeHelperTables_(sheet, stats) {
  ...
  const tables = [
    ...
    { title: 'RECORD STATUS', headers: ['STATUS', 'TOTAL'], rows: stats.recordStatus, colOffset: 14 }
  ];
...
function buildCharts_(sheet, stats) {
  ...
  const recordRange = sheet.getRange(startRow + 1, startCol + 14, Math.max(stats.recordStatus.length + 1, 2), 2);
...
function removeDashboardEditTriggers_() {
  ...
  triggers.forEach(trigger => {
    ...
    if (handler === 'handleDashboardEdit' || handler === 'onEdit') {
...
function enforceOpenSecurity_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const dashboardSheet = ss.getSheetByName(DASHBOARD_CONFIG.dashboardSheetName);
  const mPanoraSheet = ss.getSheetByName(DASHBOARD_CONFIG.sourceSheetName);

  applyDashboardProtection_();
  applyMPanoraProtection_();

  if (mPanoraSheet && !mPanoraSheet.isSheetHidden()) {
    mPanoraSheet.hideSheet();
  }
...
function applyMPanoraProtection_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(DASHBOARD_CONFIG.sourceSheetName);
  if (!sheet) throw new Error("M Panora sheet was not found.");
  ...
  const protection = sheet.protect().setDescription('Lock m panora sheet');
