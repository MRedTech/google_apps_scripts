...
function writeHelperTables_(sheet, stats) {
  ...
  const tables = [
    ...
    { title: 'TOWER DISTRIBUTION', headers: ['GROUP CATEGORY', 'Tower A', 'ANNOTATION', 'Tower B', 'ANNOTATION'], rows: addTowerAnnotationRows_(stats.towerDistribution.slice(1)), colOffset: 14, width: 5 },
    { title: 'RECORD STATUS', headers: ['STATUS', 'TOTAL'], rows: stats.recordStatus, colOffset: 20 }
  ];
  ...
  tables.forEach(table => {
    ...
    if (table.rows && table.rows.length) {
      ...
      } else if (table.title === 'TOWER DISTRIBUTION') {
        sheet.getRange(startRow + 2, col + 1, table.rows.length, 1).setNumberFormat('0');
        sheet.getRange(startRow + 2, col + 2, table.rows.length, 1).setNumberFormat('@');
        sheet.getRange(startRow + 2, col + 3, table.rows.length, 1).setNumberFormat('0');
        sheet.getRange(startRow + 2, col + 4, table.rows.length, 1).setNumberFormat('@');
      } else if (width > 1) {
...
function buildCharts_(sheet, stats) {
  ...
  const comparisonRange = sheet.getRange(startRow + 1, startCol + 14, stats.towerDistribution.length, 5);
  const recordRange = sheet.getRange(startRow + 1, startCol + 20, Math.max(stats.recordStatus.length + 1, 2), 2);
  ...
  const charts = [
    ...
    sheet.newChart()
      ...
      .setOption('annotations', {
        ...
        textStyle: { fontSize: 10, color: '#1b5e20', auraColor: 'none' }
      })
      .setOption('hAxis', {
