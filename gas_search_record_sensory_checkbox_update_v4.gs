...
function setSearchRecordActionBusy_(sheet, action) {
  ...
  if (action === 'SEARCH') {
    searchBox.setValue('TRUE').setBackground('#ffffff').setBorder(false, false, false, false, false, false);
    clearBox.clearDataValidations().clearContent().setBackground('#ffffff').setBorder(false, false, false, false, false, false);
  } else {
    clearBox.setValue('TRUE').setBackground('#ffffff').setBorder(false, false, false, false, false, false);
    searchBox.clearDataValidations().clearContent().setBackground('#ffffff').setBorder(false, false, false, false, false, false);
  }
}
...
function handleSearchRecordCheckboxEdit_(e) {
  ...
  const editedValue = String(e.value || '').trim();

  // Custom checkbox values are TRUE/FALSE text.
  // This prevents Google Sheets from showing a data-validation error when the tablet keyboard sends FALSE.

  // If the user deletes the action cell by mistake, restore the checkbox only.
  if (!editedValue) {
    restoreSearchRecordActionCheckboxes_(sheet);
  ...
  try {
    ...
    // Accept both mouse/touch checkbox tick (TRUE) and keyboard Enter edit (FALSE/text).
