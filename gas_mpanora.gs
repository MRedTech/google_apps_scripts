...
function createSectionFrames_(sheet) {
  const sections = [
    { title: 'REGISTRATION BY GROUP CATEGORY', range: 'I5:L15', titleCell: 'I5' },
...
function writeTowerGroupCategoryVisible_(sheet, groupCategorySummary) {
  ...
  sheet.getRange(r, startCol, 1, totalCols)
    .merge()
    .setValue('Note: Total under Tower Summary may be higher than Total Registration because records tagged as "Tower A & B" are counted under both towers for distribution analysis.')
    .setFontSize(8)
    .setFontStyle('italic')
    .setFontColor('#616161')
    .setWrap(true)
