...
function renderSearchRecord_(sheet, result) {
  ...
  if (values.length) {
    ...
    // Result alignment:
    // TIMESTAMP/NAME/CONTACT/REASON = left
    // MYKAD-PASSPORT / REGISTRATION NUMBER / CATEGORY / TOWER / PHOTO LINK = center
    ...
    sheet.getRange(startRow, SEARCH_RECORD_CONFIG.historyRightStartCol + 1, values.length, 1)
      .setHorizontalAlignment('center');

    sheet.getRange(startRow, SEARCH_RECORD_CONFIG.historyRightStartCol + 2, values.length, 1)
      .setHorizontalAlignment('center');

    sheet.getRange(startRow, SEARCH_RECORD_CONFIG.historyRightStartCol + 4, values.length, 1)
      .setHorizontalAlignment('center');
