...
function buildDashboard() {
  ...
  if (!sourceSheet) throw new Error(`Source sheet '${DASHBOARD_CONFIG.sourceSheetName}' was not found.`);
...
function applySensoryProtection_() {
  ...
  if (!sheet) throw new Error(`Source sheet '${DASHBOARD_CONFIG.sourceSheetName}' was not found.`);
  ...
  const protection = sheet.protect().setDescription(`Lock ${DASHBOARD_CONFIG.sourceSheetName} sheet`);
