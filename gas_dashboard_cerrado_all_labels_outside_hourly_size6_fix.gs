const DASHBOARD_CONFIG = {
  sourceSheetName: 'CERRADO',
  dashboardSheetName: 'DASHBOARD',
  timezone: 'Asia/Kuala_Lumpur',
  headerTitle: 'SECURE ENTRY DASHBOARD | CERRADO',
...
function buildDashboard() {
  ...
  if (!sourceSheet) throw new Error("Source sheet 'CERRADO' was not found.");
...
function applyCerradoProtection_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(DASHBOARD_CONFIG.sourceSheetName);
  if (!sheet) throw new Error("Cerrado sheet was not found.");
  ...
  const protection = sheet.protect().setDescription('Lock cerrado sheet');
